...
extract_ip_range(struct ip_range *r)
{
	...
	*nr = '\0';
    ...
    if (nr) {
	r->n_addrs = atoi(nr);
	r->end.s_addr = ntohl(r->start.s_addr);
	r->end.s_addr += r->n_addrs;
	r->end.s_addr = htonl(r->end.s_addr);
	D("%s %d %s %x", nr, r->n_addrs, inet_ntoa(r->end),
	    r->end.s_addr);
    } else {
...
#endif
    strcpy(s, inet_ntoa(r->start));
    strcpy(s+64, inet_ntoa(r->end));
    D("%s starts at %s, end %s %x",
      r->name, s, s+64, r->end.s_addr);
}
...
static void
init_pkts(struct targ *targ)
{
    struct ether_header *eh;
    struct ip *ip;
    struct udphdr *udp;
    uint16_t paylen = targ->g->pkt_size - sizeof(*eh) - sizeof(struct ip);
    int i, l, j, l0 = strlen(default_payload);

    struct in_addr dstip = targ->g->dst_ip.start;
    dstip.s_addr = ntohl(dstip.s_addr);

    targ->npkts = targ->g->dst_ip.n_addrs;
    targ->pkts = (struct pkt*)malloc(
	targ->npkts*sizeof(struct pkt));

    assert(targ->pkts != 0);

    for (j=0; j<targ->npkts; j++) {
	struct pkt *pkt = targ->pkts+j;
	...
	ip->ip_ttl = IPDEFTTL;
	ip->ip_p = IPPROTO_UDP;
	ip->ip_dst.s_addr = htonl(dstip.s_addr);
...
static int
send_pkts(struct netmap_ring *ring, struct pkt *pkt, 
	  int size, u_int count, int options, int idx, int npkts)
{
    u_int sent, cur = ring->cur;
...
#endif
    for (sent = 0; sent < count; sent++, idx++) {
	struct netmap_slot *slot = &ring->slot[cur];
	char *p = NETMAP_BUF(ring, slot->buf_idx);

	if (options & OPT_COPY)
	    pkt_copy(pkt+(idx%npkts), p, size);
	else if (options & OPT_MEMCPY)
	    memcpy(p, pkt+(idx%npkts), size);
	else if (options & OPT_PREFETCH)
	    prefetch(p);
	slot->len = size;
	if (sent == count - 1)
